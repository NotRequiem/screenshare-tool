...
> 15. Detects mods used by the game modified while the game was running.
...
3. Detections for mods modified after the last javaw boot.

4. Detections for client-specific strings will not be added for two main reasons:
    ...
    - Why I would add a large database if the tool can detect any cheat with generic methods?
...
8. The tool will not exploit signed and vulnerable kernel drivers, or use any kernel driver to scan memory in "protected" processes such as csrss, because the need of a digital signature to not be blocked by Windows.

9. The tool does not detect any dll injection using Registry, prefetch, memory images, system snapshots or processes hosted by antivirus with memory persistence, due to their lack of reliability.

10. The tool doesn't need to detect deleted or renamed files, because the tool already checks for this when proving file execution.

11. Since the tool is just designed for Windows, there are no CMake files to simplify the build process for everyone, as you just have to download Visual Studio, open the solution file and click on Build.
