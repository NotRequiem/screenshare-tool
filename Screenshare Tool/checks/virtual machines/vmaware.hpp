 ...
 *
 *  - Made by: @kernelwernel (https://github.com/kernelwernel) and @NotRequiem (https://github.com/NotRequiem)
...
#include <sys/sysctl.h>
    ...
    private:
...
#if (MSVC)
            i32 x[4]{};
            __cpuidex((i32*)x, a_leaf, c_leaf); // no need to explicitly cast
        ...
        [[nodiscard]] static u32 get_disk_size() {
            ...
            if (size == 0) { // here cppcheck may false flag with "Condition 'size==0' is always true [knownConditionTrueFalse]"
                return false;
            }
        ...
        [[nodiscard]] static u64 get_physical_ram_size() {
            ...
            return (total_memory_kb / (static_cast<unsigned long long>(1024) * 1024)); // 1000?
#else
            return 0;
#endif
        }
...
#define LODWORD(_qw)    ((DWORD)(_qw))
        ...
        [[nodiscard]] static bool mac_address_check() try {
            ...
            AdapterInfo = (IP_ADAPTER_INFO*)std::malloc(sizeof(IP_ADAPTER_INFO)); // no need to explicitly cast
            ...
            if (GetAdaptersInfo(AdapterInfo, &dwBufLen) == ERROR_BUFFER_OVERFLOW) {
                ...
                AdapterInfo = (IP_ADAPTER_INFO*)std::malloc(dwBufLen); // no need to explicitly cast
                if (AdapterInfo == NULL) {
                    std::free(mac_addr);
                    return false;
                }
        ...
        [[nodiscard]] static bool user_check() try {
            ...
            GetUserName((TCHAR*)user, &user_len); // no need to explicitly cast
        ...
        [[nodiscard]] static bool vm_files() try {
            ...
            else if (vbox == vmware) { // here cppcheck may false flag and warn about "Condition 'vbox==vmware' is always true [knownConditionTrueFalse]"
...
#elif (MSVC)
            double ret = 0.0;
            NTSTATUS(WINAPI * RtlGetVersion)(LPOSVERSIONINFOEXW) = nullptr; // Initialize to nullptr
            OSVERSIONINFOEXW osInfo{};

            HMODULE ntdllModule = GetModuleHandleA("ntdll");
            if (ntdllModule != nullptr) {
                *(FARPROC*)&RtlGetVersion = GetProcAddress(ntdllModule, "RtlGetVersion"); // no need to explicitly cast
        ...
        [[nodiscard]] static bool vm_processes() try {
            ...
            auto check_proc = [](const WCHAR* proc) -> bool {
                ...
                if (Process32First(hSnapshot, &pe)) {
                    do {
                        std::wstring wc(MAX_PATH, L'#');
                        size_t convertedChars = 0;
                        mbstowcs_s(&convertedChars, &wc[0], MAX_PATH, pe.szExeFile, _TRUNCATE);
            ...
            auto ret = [](const WCHAR* str) -> bool {
#ifdef __VMAWARE_DEBUG__
                debug(L"VM_PROCESSES: found ", str);
#endif
                ...
                WideCharToMultiByte(CP_UTF8, 0, str, -1, &narrowStr[0], size, nullptr, nullptr);

                return add(narrowStr.c_str());
                };
