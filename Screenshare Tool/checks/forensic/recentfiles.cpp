...
void RecentFiles(bool imp) {
    ...
    std::vector<std::wstring> substrings = { L".jar", L".bat", L".vbs", L".py", L".ps1", L".dll", L".go" };
    do {
        if ((findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) {
            std::wstring wideFileName = findFileData.cFileName;
            if (!screenshare_tool::FileTracker::isFileProcessed(wideFileName) && ContainsSubstring(wideFileName, substrings) &&
                ...
                screenshare_tool::FileTracker::addProcessedFile(wideFileName);
            }
        }
