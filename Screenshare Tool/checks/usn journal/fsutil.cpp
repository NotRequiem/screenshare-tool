...
static void compareHashTables(FileData** hashTable1, int tableSize1, FileData** hashTable2, int tableSize2, bool imp) {
    if (!imp) {
 ...
 *
 */
static void processLogFileComparison(bool imp) {
    ...
    compareHashTables(hashTable1, tableSize1, hashTable2, tableSize2, imp);
...
static void RunCommand(TCHAR driveLetter, bool scanMacroTraces) {
    ...
    if (scanMacroTraces) {
        ...
        _tprintf(_T("[NTFS Scanner] Detecting renamed and deleted Prefetch folders on the entire NTFS drive...\n"));
        _stprintf_s(command, _T("fsutil usn readjournal %c: csv | findstr /i /C:\"Prefetch\" >> .\\prefetch.txt"), driveLetter);
        system(command);
    }
    else {
