...
static bool IsProcessRestartedSinceLastLogon(DWORD processId, const SYSTEMTIME& lastLogonTime) {
    ...
    if (GetProcessCreationTime(processId, creationTime)) {
        ...
        static bool isLastLogonTimePrinted = false;

        if (!isLastLogonTimePrinted) {
            std::wcout << L"[#] Last Logon Time: " << localLastLogonSystemTime.wYear << L"/" << localLastLogonSystemTime.wMonth
                << L"/" << localLastLogonSystemTime.wDay << L" " << localLastLogonSystemTime.wHour << L":"
                << localLastLogonSystemTime.wMinute << L":" << localLastLogonSystemTime.wSecond << std::endl;

            isLastLogonTimePrinted = true;
        }
