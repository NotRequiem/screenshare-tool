...
static void SearchForDurationMS(const wchar_t* logFilePath) {
    ...
    if (file != NULL) {
        ...
        if ((err = _wfopen_s(&file, logFilePath, L"r")) == 0) {
            ...
            // Handle the error
            if (err == EACCES) {
                wprintf(L"Error: Permission denied when accessing the macro file: %s when checking for macro traces. Manually check if this file contains the string 'durationMs.'\n", logFilePath);
            }
        }
    }
}
    ...
    if (_wfopen_s(&file, logFilePath, L"r") == 0) {
        ...
        while (file != NULL && fgetws(line, sizeof(line) / sizeof(line[0]), file) != NULL) {
            if (wcsstr(line, searchString) != NULL) {
                wprintf(L"[!] Razer Macro detected and deleted: %s. Ban the player.", line);
            }
        }
...
static bool CheckFileModification(const wchar_t* filePath) {
    ...
    if (GetFileAttributesExW(filePath, GetFileExInfoStandard, &fileData)) {
        ...
        wprintf(L"[!] Macro file modification time: %d/%d/%d %d:%d:%d. Ban the player if this modification time is just after the time you freezed the player.\n",
...
static void CheckFilesInFolder(const wchar_t* folderPath, const wchar_t* extension) {
    ...
    do {
        ...
        if (CheckFileModification(fullPath)) {
            wprintf(L"[!] Macro file %s was recently modified!. Ban the player if this modification time is just after the time you freezed the player..\n", fullPath);
        }
