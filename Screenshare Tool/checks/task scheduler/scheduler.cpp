    ...
    if (CreateProcessW(NULL, const_cast<wchar_t*>(commandLine.c_str()), NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
        ...
        while (ReadFile(hChildStdoutRd, buffer, sizeof(buffer), &bytesRead, NULL) && bytesRead != 0) {
            ...
            while (std::getline(outputStream, line)) {
                ...
                std::vector<std::string> fileExtensions = { ".exe", ".dll", ".bat", ".vbs", ".jar", ".ps1", ".py", ".go" };
                ...
                if (!line.empty() && printedPaths.find(line) == printedPaths.end() && std::regex_search(line, match, SchedulerRegex)) {
                    ...
                    std::wstring wideMatchedPart;
                    int requiredSize = MultiByteToWideChar(CP_UTF8, 0, matchedPart.c_str(), -1, nullptr, 0);
                    if (requiredSize > 0) {
                        wideMatchedPart.resize(static_cast<std::wstring::size_type>(requiredSize) - 1);  // Exclude null terminator
                        MultiByteToWideChar(CP_UTF8, 0, matchedPart.c_str(), -1, &wideMatchedPart[0], requiredSize);
                    }

                    if (!screenshare_tool::FileTracker::isFileProcessed(wideMatchedPart)) {
                        if (fs::exists(wideMatchedPart)) {
                            if (!IsFileSignatureValid(wideMatchedPart)) {
                                std::wcout << "[#] Executed & Unsigned file: " << wideMatchedPart << std::endl;
                            }
                        }
                        else {
                            std::wcout << "[#] Executed & Deleted file: " << wideMatchedPart << std::endl;
                        }

                        // Add to the FileTracker to avoid duplicate output
                        screenshare_tool::FileTracker::addProcessedFile(wideMatchedPart);
                    }
                }
