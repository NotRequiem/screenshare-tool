...
static void ExtractFilePath(const std::string& line) {
    ...
    if (startPos != std::string::npos && endPos != std::string::npos) {
        ...
        if (!screenshare_tool::FileTracker::isFileProcessed(wideFilePath)) {
            if (fs::exists(wideFilePath)) {
                if (!IsFileSignatureValid(wideFilePath)) {
                    std::wcout << L"[#] Executed & Unsigned file: " << wideFilePath << std::endl;
                    screenshare_tool::FileTracker::addProcessedFile(wideFilePath);
                }
            }
            else {
        ...
        while (std::getline(outputStream, line)) {
            ...
            if (wideSize > 0) {
                ...
                if (volumePos != std::wstring::npos) {
                    ...
                    std::wstring wideFilePath(wideStr.begin(), wideStr.end());
                    if (!screenshare_tool::FileTracker::isFileProcessed(wideFilePath)) {
                        // Check if the file exists
                        if (std::filesystem::exists(wideFilePath)) {
                            if (!IsFileSignatureValid(wideFilePath)) {
                                std::wcout << L"[#] Executed & Unsigned file: " << wideFilePath << std::endl;
                                screenshare_tool::FileTracker::addProcessedFile(wideFilePath);
                            }
...
static void DetectJarsAndBats(DWORD pid) {
    ...
    std::wregex regexPattern(LR"(^(\\|[A-Z]:\\)(?![^\\]*\.exe).*\..*"\s*$)");
    ...
    if (CreateProcessW(NULL, const_cast<wchar_t*>(commandLine.c_str()), NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
        ...
        while (ReadFile(hChildStdoutRd, buffer, sizeof(buffer), &bytesRead, NULL) && bytesRead != 0) {
            std::wstring outputString(buffer, buffer + std::strlen(buffer));

            // Use wistringstream to parse the output into lines
            std::wistringstream outputStream(outputString);
            std::wstring line;
            ...
            while (std::getline(outputStream, line)) {
                ...
                for (const auto& patternAndMessage : patternsAndMessages) {
                    ...
                    if (!screenshare_tool::FileTracker::isFileProcessed(line)) {
                        // Use find on std::wstring
                        if (line.find(wideSearchPattern) != std::wstring::npos) {
                            std::wcout << message << line << std::endl;
                            foundMatch = true;
...
void ExecutedFiles(bool imp) {
    if (!imp) {
        ...
        std::cout << "[Memory Scanner] Running checks to detect executed files in memory...\n";
    ...
    if (!imp) {
        ...
        setConsoleTextColor(BrightYellow);
        std::cout << "[Memory Scanner] Analyzing process " << "Explorer\n";
        resetConsoleTextColor();
    }
