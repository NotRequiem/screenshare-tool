...
void csrss() {
    ...
    filePath2 = PromptForFilePath(L"After doing this, enter here the full path of the results file. Type 'cancel' to exit this check if you cant dump csrss or there's no strings inside it: ");
    if (filePath2.empty()) {
        return;  // User canceled, skip to the NTFS Scanner module
    }
