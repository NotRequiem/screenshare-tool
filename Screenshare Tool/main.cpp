...
#include "vmaware.hpp"
#include "Macros\process_scanner.hpp"
...
int main(int argc, char* argv[]) {
    ...
    // Define the process names and corresponding regular expressions
    std::vector<std::pair<std::string, std::string>> processPatterns = {
        {"lghub_agent.exe", "durationms.+\"isDown\""},
        {"Razer Synapse.exe", "DeleteMacroEvent"},
        {"Razer Synapse 3.exe", "SetKeysPerSecond"},
        {"RazerCentralService.exe", "Datasync: Status: COMPLETE Action: NONE Macros/"},
        {"SteelSeriesGGClient.exe", "delay.+is_deleted"},
        {"Onikuma.exe", "LeftKey CODE:"}
    };

    for (const auto& pair : processPatterns) {
        const std::string& processName = pair.first;
        const std::string& patternStr = pair.second;
        std::regex pattern(patternStr);

        scanProcessStrings(processName.c_str(), pattern);
    }

    bool running = IsBAMRunning();
    if (!running) {
        std::cout << "The Background Activity Monitor driver is not running, therefore checks for modified extensions, files without extensions and files without names won't run." << std::endl;
    }
    ...
    if (!ntfsDrives.empty()) {
        for (const std::wstring& drive : ntfsDrives) {
            // Get the current directory and create the full path to MFTECmd.exe
    ...
    // Iterate through available drive letters and check the journal for each drive
