    ...
    *tableSize = 0;
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        if (strstr(line, "Rename:") != NULL) {
            ...
            FileData** tempTable = realloc(*hashTable, (*tableSize + 1) * sizeof(FileData*));
            if (tempTable == NULL) {
                // Handle the case where realloc fails (returns NULL)
                perror("Error reallocating memory");
                // Optionally, you might want to free the original memory here
                // free(*hashTable);
                exit(EXIT_FAILURE);
            }
            *hashTable = tempTable;
            ...
            if ((*hashTable)[*tableSize] == NULL) {
                // Handle the case where malloc fails (returns NULL)
                perror("Error allocating memory");
                // Optionally, you might want to free the memory allocated by realloc here
                // free(*hashTable);
                exit(EXIT_FAILURE);
            }
            ...
            if (positions[5] != NULL) {
                ...
                if (positions[2] != NULL) {
                    strncpy_s((*hashTable)[*tableSize]->fileName, _countof((*hashTable)[*tableSize]->fileName), (positions[1] != NULL && positions[2] != NULL) ? positions[1] + 1 : "", (positions[1] != NULL && positions[2] != NULL) ? positions[2] - positions[1] - 1 : 0);
    ...
    for (int i = 0; i < tableSize1; i++) {
        for (int j = 0; j < tableSize2; j++) {
            if (strcmp(hashTable1[i]->fileName, hashTable2[j]->fileName) == 0 &&
                ...
                if (!alreadyPrinted) {
                    // Use a temporary pointer to avoid potential memory leaks
                    FileData* tempLines = realloc(printedLines, (numPrintedLines + 1) * sizeof(FileData));
                    if (tempLines == NULL) {
                        // Handle the case where realloc fails (returns NULL)
                        perror("Error reallocating memory to detect file replaces.");
                        // Free the original memory here before exiting
                        free(printedLines);
                        exit(EXIT_FAILURE);
                    }
...
void LastBootTime(SYSTEMTIME* lastBootTime) {
