...
static void CheckDriveJournal(const std::wstring& driveLetter) {
    ...
    if (hFile != INVALID_HANDLE_VALUE && GetFileInformationByHandle(hFile, &fileInformation)) {
        ...
        // Print modification time and boot time
        std::wcout << L"Modification Time: "
            << localModificationTime.wYear << L"/" << localModificationTime.wMonth << L"/" << localModificationTime.wDay
            << L" " << localModificationTime.wHour << L":" << localModificationTime.wMinute << L":" << localModificationTime.wSecond << std::endl;

        std::wcout << L"Boot Time: "
            << startTime.wYear << L"/" << startTime.wMonth << L"/" << startTime.wDay
            << L" " << startTime.wHour << L":" << startTime.wMinute << L":" << startTime.wSecond << std::endl;
    }
    else {
