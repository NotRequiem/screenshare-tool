...
static void buildHashTable(FILE* file, FileData*** hashTable, int* tableSize, SYSTEMTIME lastBootTime) {
    ...
    *tableSize = 0;
    ...
    _tprintf(_T("[NTFS Scanner] Reconstructing hash table to detect replaced files...\n"));
    Console::ResetColor();
...
static void RunCommand(TCHAR driveLetter) {
    ...
    _tprintf(_T("[NTFS Scanner] Checking modification traces...\n"));
    Console::ResetColor();
...
void USNJournal() {
    Console::SetColor(ConsoleColor::Cyan, ConsoleColor::Black);
    _tprintf(_T("[NTFS Scanner] Analyzing drives structure...\n"));
    ...
    _tprintf(_T("[NTFS Scanner] Order the folder by \"Date modified\" and open the most recent \".txt\" files.\n"));
    Console::ResetColor();
