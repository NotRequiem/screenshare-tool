#include "D:\Screenshare Tool\usn journal\fsutil.hpp"
...
static void buildHashTable(FILE* file, FileData*** hashTable, int* tableSize, SYSTEMTIME lastBootTime) {
    ...
    *tableSize = 0;
    _tprintf(_T("Reconstructing hash table to detect replaced files...\n"));
...
static void compareHashTables(FileData** hashTable1, int tableSize1, FileData** hashTable2, int tableSize2) {
    _tprintf(_T("Detecting replaced files on the entire NTFS drive...\n"));
    FILE* outputFile;
...
static void LastBootTime(SYSTEMTIME* lastBootTime) {
    ...
    printf("Computer started on: %02d/%02d/%04d %02d:%02d:%02d\n",
        lastBootTime->wMonth, lastBootTime->wDay, lastBootTime->wYear,
...
int ConvertWideCharToMultiByte(const wchar_t* wideStr, char* multiByteStr, size_t size) {
...
void USNJournal() {
    _tprintf(_T("Analyzing drives structure...\n"));
