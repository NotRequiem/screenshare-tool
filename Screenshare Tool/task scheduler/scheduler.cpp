    ...
    while (std::getline(inputFile, line)) {
        if (std::regex_search(line, matches, pattern)) {
            ...
            if (!IsFileSignatureValid(widePath)) {
                std::cout << "Executed & Unsigned file executed with task scheduler: " << matchedPath << std::endl;
            }
        }
    }
...
static void DetectTaskScheduler(DWORD pid) {
    std::string command = "memory.exe -p " + std::to_string(pid) + " > process_strings.txt";
...
static USN_JOURNAL_DATA QueryJournal(HANDLE hVol) {
...
static DWORDLONG GetFRNForTasksDirectory(HANDLE hVol) {
...
static void CheckDeletedTasksInUSNJournal(HANDLE hVol, const USN_JOURNAL_DATA& journal) {
    ...
    std::time_t thresholdTime = std::time(nullptr) - static_cast<long long>(30 * 24 * 60) * 60;
