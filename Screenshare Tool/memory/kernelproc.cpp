...
// Function to compare the user input in a case-insensitive manner
...
static bool IsLineInDeletedFile(const std::wstring& lineToCheck) {
...
void csrss() {
    ...
    std::wcout << L"[NTFS Scanner] Analyzing deleted and renamed files registered by usn journal before running checks for files executed with modified extensions..." << std::endl;
    Console::ResetColor();
    ...
    while (!fileScanned) {
        ...
        std::wcout << L"[Memory Scanner] Initializing checks to detect files executed without name..." << std::endl;
        std::wcout << L"[Memory Scanner] Initializing checks to detect files executed without extension..." << std::endl;
        std::wcout << L"[Memory Scanner] Initializing checks to detect files executed with modified/spoofed extension..." << std::endl;
        std::wcout << L"[Memory Scanner] Initializing checks to detect dll injections..." << std::endl;
        Console::ResetColor();

        if (_setmode(_fileno(stdout), _O_U16TEXT) == -1) {
            std::wcout << L"The program may not detect paths with wide characters in your file path." << std::endl;
        }
        ...
        if (inputFile.is_open()) {
            ...
            std::wcerr << L"Error opening the csrss results file. Please re-enter a valid file path." << std::endl;
        }
    }
}