...
void checkMemoryExe() {
    ...
    std::string programName = "memory.exe";
    std::filesystem::path programPath = executablePath.parent_path() / programName;

    if (!std::filesystem::exists(programPath)) {
        std::cerr << "Warning: " << programName << " not found in the same path as the executable. Download it at: https://github.com/NotRequiem/memscanner/releases/download/memscanner/memory.exe" << std::endl;
    }
}
