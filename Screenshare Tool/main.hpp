...
void checkMemoryExe() {
    ...
    if (!std::filesystem::exists(programPath)) {
        std::wcerr << programName << L" not found in the same path as the executable. Download it at: https://github.com/NotRequiem/memscanner/releases/download/memscanner/memory.exe" << std::endl;
    }
}
