...
static void PrintEvent(EVT_HANDLE hEvent, SYSTEMTIME& lastBootTime) {
    ...
    if (pRenderedValues) {
        // Extract system time from the event values, if you see an IntelliSense warning here ignore it
        ...
        if (CompareSystemTimes(lastBootTime, st) < 0) {
            wprintf(L"[!] The system time was last modified in: %02d/%02d/%04d %02d:%02d:%02d. This is bannable but Windows may false flag it.\n", st.wDay, st.wMonth, st.wYear, st.wHour, st.wMinute, st.wSecond);
        }
