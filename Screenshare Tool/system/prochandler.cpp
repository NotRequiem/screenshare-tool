...
static bool IsProcessRestartedSinceLastLogon(DWORD processId, const SYSTEMTIME& lastLogonTime) {
    ...
    if (GetProcessCreationTime(processId, creationTime)) {
        ...
        // Add 10 seconds to the creation time to avoid weird false flags
        FILETIME creationTimePlus10Seconds{};
        ULARGE_INTEGER ulCreationTime{};
        ulCreationTime.LowPart = localCreationTime.dwLowDateTime;
        ulCreationTime.HighPart = localCreationTime.dwHighDateTime;
        ulCreationTime.QuadPart += static_cast<unsigned long long>(10) * 10000000; // Adding 10 seconds in 100-nanosecond intervals
        creationTimePlus10Seconds.dwLowDateTime = ulCreationTime.LowPart;
        creationTimePlus10Seconds.dwHighDateTime = ulCreationTime.HighPart;

        // Check if the adjusted creation time is after the last logon time
        if (CompareFileTime(&creationTimePlus10Seconds, &lastLogonFileTime) > 0) {
...
void RestartedProcesses() {
    ...
    std::wcout << L"[System Scanner] Checking for restarted processes...\n";
