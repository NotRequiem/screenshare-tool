...
static bool GetProcessCreationTime(DWORD processId, FILETIME& creationTime) {
    ...
    GetProcessTimes(hProcess, &dummyCreationTime, &dummyExitTime, &dummyKernelTime, &dummyUserTime);
...
static bool IsProcessRestartedSinceBoot(DWORD processId, const SYSTEMTIME& bootTime) {
    ...
    if (GetProcessCreationTime(processId, creationTime)) {
        ...
        if (FileTimeToSystemTime(&creationTime, &sysTime)) {
            ...
            // Check if the process creation time is greater than the system boot time
...
void RestartedProcesses() {
    ...
    const wchar_t* serviceNames[] = { L"PlugPlay", L"PcaSvc", L"Schedule", L"Eventlog" };
