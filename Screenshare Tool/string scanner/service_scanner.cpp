...
// Function to scan strings in a specified service process
    ...
    if (Process32FirstW(hSnapshot, &pe32)) {
        do {
            ...
            if (lstrcmpiW(pe32.szExeFile, processName) == 0) {
                ...
                std::wcout << L"[Service Scanner] Scanning process: " << processName << L" (PID: " << pid << L")" << std::endl;
                break;
            }
                        ...
                        if (ReadProcessMemory(hProcess, address, buffer, sizeof(buffer), &bytesRead)) {
                            ...
                            if (useRegex) {
                                ...
                                while (std::regex_search(dataStart, dataEnd, wmatch, pattern)) {
                                    std::wcout << L"File execution detected in " << processName << L" (PID: " << pid << L"): " << wmatch[0].str() << std::endl;
